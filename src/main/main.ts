...
const createWindow = (): void => {
      ...
      submenu: [
        ...
        { type: 'separator' },
        {
          ...
          click: async () => {
            ...
            const data = fs.readFileSync(filePaths[0], { encoding: 'utf8' })
        ...
        },
        {
          label: '新しいウィンドウを開く',
          click: async () => {
            const exePath = app.getPath("exe"); // 実行可能ファイルのパス
        ...
        { type: 'separator' },
        isMac ? { label: '閉じる', role: 'close' } : { label: '閉じる', role: 'quit' },
      ]
    },
    {
      label: "編集",
  ...
  ipcMain.on('send-editor-value', async (_event, value, isSaveAs) => {
    let saveFilePath = ""
    if (openedFilePath === undefined || openedFilePath !== "") {
      saveFilePath = openedFilePath;
    }
